from __future__ import annotations

import io
from typing import Any, TypeVar

T = TypeVar("T")

# Basic function binding
def add(a: int, b: int) -> int: ...
def add3(a: int, b: int, c: int | None = ...) -> int: ...
def add_named(a: int, b: int, c: int | None = ...) -> int: ...

# Type conversions
def identity_int(x: int) -> int: ...
def identity_float(x: float) -> float: ...
def identity_bool(x: bool) -> bool: ...
def identity_str(x: str) -> str: ...
def identity_bytes(x: bytes) -> bytes: ...
def identity_optional(x: str | None) -> str | None: ...
def identity_object(x: T) -> T: ...
def int64_or_uint64(value: int) -> tuple[bool, int]: ...
def mask_u32(value: int) -> int: ...
def mask_u64(value: int) -> int: ...
def bigint_to_string(value: int) -> str: ...
def bigint_roundtrip(value: int) -> int: ...
def int_roundtrip(value: int) -> int: ...

# Bytes operations
def bytes_len(data: bytes) -> int: ...
def bytes_slice(data: bytes, start: int, end: int) -> bytes: ...
def bytes_create(value: str) -> bytes: ...
def buffer_len(data: bytes | bytearray | memoryview) -> int: ...
def buffer_sum(data: bytes | bytearray | memoryview) -> int: ...
def bytes_view_len(data: bytes | bytearray | memoryview) -> int: ...
def bytes_view_sum(data: bytes | bytearray | memoryview) -> int: ...
def bytes_view_is_buffer(data: bytes | bytearray | memoryview) -> bool: ...
def io_read(stream: io.BytesIO, max_len: int) -> bytes: ...
def write_all(stream: io.BytesIO, data: bytes | bytearray | memoryview) -> int: ...

# List operations
def list_len(lst: list[Any]) -> int: ...
def list_get(lst: list[Any], index: int) -> Any: ...
def list_sum(values: list[int]) -> int: ...
def list_create(a: int, b: int, c: int) -> list[int]: ...
def list_append(lst: list[int], value: int) -> list[int]: ...
def list_set(lst: list[int], index: int, value: int) -> list[int]: ...

# Dict operations
def dict_len(d: dict[Any, Any]) -> int: ...
def dict_get(d: dict[str, int], key: str) -> int | None: ...
def dict_create(key: str, value: int) -> dict[str, int]: ...
def dict_set(d: dict[str, int], key: str, value: int) -> dict[str, int]: ...
def dict_keys(d: dict[Any, Any]) -> list[Any]: ...

# Tuple operations
def tuple_len(t: tuple[Any, ...]) -> int: ...
def tuple_get(t: tuple[Any, ...], index: int) -> Any: ...
def tuple_create(a: int, b: int) -> tuple[int, int]: ...
def tuple_create_manual(a: int, b: int) -> tuple[int, int]: ...

# Object operations
def obj_call0(obj: Any) -> Any: ...
def obj_call1(obj: Any, arg: Any) -> Any: ...
def obj_call2(obj: Any, arg1: Any, arg2: Any) -> Any: ...
def obj_getattr(obj: Any, name: str) -> Any: ...
def obj_setattr(obj: Any, name: str, value: Any) -> bool: ...
def obj_callmethod0(obj: Any, name: str) -> Any: ...
def obj_callmethod1(obj: Any, name: str, arg: Any) -> Any: ...
def obj_is_callable(obj: Any) -> bool: ...
def obj_is_none(obj: Any) -> bool: ...

# Type checking
def is_unicode(obj: Any) -> bool: ...
def is_bytes(obj: Any) -> bool: ...
def is_bool(obj: Any) -> bool: ...
def is_int(obj: Any) -> bool: ...
def is_float(obj: Any) -> bool: ...
def is_list(obj: Any) -> bool: ...
def is_tuple(obj: Any) -> bool: ...
def is_dict(obj: Any) -> bool: ...

# Error handling
def raise_type_error() -> None: ...
def raise_value_error() -> None: ...
def raise_runtime_error() -> None: ...
def raise_zero_division() -> None: ...
def raise_overflow_error() -> None: ...
def raise_attribute_error() -> None: ...
def raise_index_error() -> None: ...
def raise_key_error() -> None: ...
def raise_memory_error() -> None: ...
def divide(a: float, b: float) -> float: ...
def raise_mapped(kind: str) -> None: ...
def get_del_count() -> int: ...
def reset_del_count() -> None: ...

# Python interop
def import_math_pi() -> float: ...
def call_upper(value: str) -> str: ...

# Class wrapper checks
def baseadder_expect(obj: Any) -> bool: ...
def baseadder_expect_exact(obj: Any) -> bool: ...
def payloadbox_expect(obj: Any) -> bool: ...
def payloadbox_payload_from(obj: Any) -> int: ...

# Module attributes
VERSION: str
DEFAULT_SIZE: int
ENABLED: bool
OPTIONAL: str | None
PI: float

# Classes
class BaseAdder:
    def __init__(self) -> None: ...
    def add(self, a: int, b: int) -> int: ...
    def identity(self) -> BaseAdder: ...

class Adder:
    def __init__(self) -> None: ...
    def add(self, a: int, b: int) -> int: ...
    def identity(self) -> Adder: ...

class Counter:
    def __init__(self) -> None: ...
    def get(self) -> int: ...
    def increment(self) -> int: ...
    def add(self, value: int) -> int: ...
    def reset(self) -> None: ...

class PayloadBox:
    def __init__(self, value: int) -> None: ...
    def get(self) -> int: ...
    def set(self, value: int) -> None: ...

class MethodKinds:
    def __init__(self) -> None: ...
    @classmethod
    def class_name(cls) -> str: ...
    @staticmethod
    def sum(a: int, b: int) -> int: ...

class CallableAdder:
    def __init__(self) -> None: ...
    def __call__(self, value: int, extra: int | None = ...) -> int: ...

class DunderBasics:
    value: int
    def __init__(self) -> None: ...
    def __repr__(self) -> str: ...
    def __str__(self) -> str: ...
    def __len__(self) -> int: ...
    def __hash__(self) -> int: ...
    def __bool__(self) -> bool: ...

class SubscriptBox:
    def __init__(self) -> None: ...
    def __getitem__(self, key: str) -> int: ...
    def __setitem__(self, key: str, value: int) -> None: ...
    def __delitem__(self, key: str) -> None: ...

class ComparePoint:
    value: int
    def __init__(self) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __lt__(self, other: ComparePoint) -> bool: ...
    def __le__(self, other: ComparePoint) -> bool: ...
    def __gt__(self, other: ComparePoint) -> bool: ...
    def __ge__(self, other: ComparePoint) -> bool: ...
    def __ne__(self, other: object) -> bool: ...

class NumberBox:
    value: int
    def __init__(self) -> None: ...
    def __add__(self, other: NumberBox) -> int: ...
    def __sub__(self, other: NumberBox) -> int: ...
    def __mul__(self, factor: int) -> int: ...
    def __truediv__(self, divisor: float) -> float: ...
    def __floordiv__(self, divisor: int) -> int: ...
    def __mod__(self, divisor: int) -> int: ...
    def __pow__(self, exponent: int, mod: int | None = ...) -> int: ...
    def __divmod__(self, divisor: int) -> tuple[int, int]: ...
    def __matmul__(self, other: NumberBox) -> int: ...
    def __neg__(self) -> int: ...
    def __pos__(self) -> int: ...
    def __abs__(self) -> int: ...
    def __invert__(self) -> int: ...
    def __and__(self, other: int) -> int: ...
    def __or__(self, other: int) -> int: ...
    def __xor__(self, other: int) -> int: ...
    def __lshift__(self, other: int) -> int: ...
    def __rshift__(self, other: int) -> int: ...
    def __int__(self) -> int: ...
    def __float__(self) -> float: ...
    def __index__(self) -> int: ...
    def __iadd__(self, other: int) -> NumberBox: ...
    def __isub__(self, other: int) -> NumberBox: ...
    def __imul__(self, other: int) -> NumberBox: ...
    def __itruediv__(self, other: int) -> NumberBox: ...
    def __ifloordiv__(self, other: int) -> NumberBox: ...
    def __imod__(self, other: int) -> NumberBox: ...
    def __ipow__(self, exponent: int, mod: int | None = ...) -> NumberBox: ...
    def __iand__(self, other: int) -> NumberBox: ...
    def __ior__(self, other: int) -> NumberBox: ...
    def __ixor__(self, other: int) -> NumberBox: ...
    def __ilshift__(self, other: int) -> NumberBox: ...
    def __irshift__(self, other: int) -> NumberBox: ...
    def __imatmul__(self, other: NumberBox) -> NumberBox: ...

class ContextLock:
    entered: bool
    exited: bool
    def __init__(self) -> None: ...
    def __enter__(self) -> ContextLock: ...
    def __exit__(
        self,
        exc_type: object | None,
        exc_val: object | None,
        exc_tb: object | None,
    ) -> bool: ...

class IterCounter:
    limit: int
    current: int
    def __init__(self) -> None: ...
    def __iter__(self) -> IterCounter: ...
    def __next__(self) -> int: ...

class ContainsBox:
    items: list[int]
    def __init__(self) -> None: ...
    def __contains__(self, value: int) -> bool: ...

class InitBox:
    value: int
    def __init__(self, value: int) -> None: ...

class AttrAccessBox:
    value: int
    shadowed: str
    missing: str
    def __init__(self) -> None: ...
    def __getattribute__(self, name: str) -> Any: ...
    def __getattr__(self, name: str) -> Any: ...

class AttrSetBox:
    value: str
    def __init__(self) -> None: ...
    def __setattr__(self, name: str, value: Any) -> None: ...
    def __delattr__(self, name: str) -> None: ...

class DescriptorBox:
    def __init__(self) -> None: ...
    def __get__(self, obj: object | None, owner: type | None) -> Any: ...
    def __set__(self, obj: object, value: Any) -> None: ...
    def __delete__(self, obj: object) -> None: ...

class NewBox:
    value: int
    def __new__(cls, value: int) -> NewBox: ...

class DelBox:
    value: int
    def __init__(self) -> None: ...
    def __del__(self) -> None: ...

__all__: list[str]
